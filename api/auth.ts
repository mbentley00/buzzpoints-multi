...
import {
  loadUsers, saveUsers, hashPassword, verifyPassword, signToken, signPurpose, readPurpose,
  setSessionCookie, clearSessionCookie, currentUser, normEmail, isAdminEmail, isVerified, roleOf, User,
} from "./_lib/auth.js";
import { sendEmail, appUrl, verifyEmailBody } from "./_lib/email.js";

const VERIFY_TTL = 60 * 60 * 24; // 24h
const verifyUrl = (email: string) => `${appUrl()}/verify?token=${encodeURIComponent(signPurpose(email, "verify", VERIFY_TTL))}`;

async function sendVerification(email: string, name: string) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === "verify") {
      const email = readPurpose(String((req.body || {}).token || ""), "verify");
      ...
      return res.status(200).json({ email, name: u.name ?? null, isAdmin: isAdminEmail(email), role: roleOf(email, users) });
    }
    ...
    if (action === "resend-verification") {
      ...
      if (u && u.verified === false) { const { devUrl } = await sendVerification(email, u.name); return res.status(200).json({ ok: true, devUrl }); }
    ...
    if (action === "signup") {
      ...
      const { delivered, devUrl } = await sendVerification(email, name);
    ...
    if (action === "login") {
      ...
      if (!isVerified(u)) {
        const { devUrl } = await sendVerification(email, u.name);
