...
export function CategoriesTossup() {
  ...
  const columns: CatColumn<CatTossupRow, CatTossupSub>[] = [
    ...
    { label: "Conv%", align: "right", main: (g) => pct(g.convPct), sub: (s) => pct(s.convPct) },
    ...(meta.hasPower
      ? [{ label: "Pwr%", align: "right" as const, main: (g: CatTossupRow) => pct(g.powerPct), sub: (s: CatTossupSub) => pct(s.powerPct) }]
      : []),
    { label: "Inc%", align: "right", title: "Rate of interrupting incorrect buzzes per play", main: (g) => pct(g.incorrectPct), sub: (s) => pct(s.incorrectPct) },
    { label: "1st Sent%", align: "right", title: "Share of plays converted by a correct buzz within the first sentence", main: (g) => pct(g.firstSentConvPct), sub: (s) => pct(s.firstSentConvPct) },
    { label: "2nd Sent%", align: "right", title: "Share of plays converted by the end of the second sentence", main: (g) => pct(g.secondSentConvPct), sub: (s) => pct(s.secondSentConvPct) },
    { label: "Avg Buzz", align: "right", main: (g) => buzz(g.avgBuzzPct), sub: (s) => buzz(s.avgBuzzPct) },
